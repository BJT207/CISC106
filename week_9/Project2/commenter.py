...
class Line:
    ...
    Attributes:
        ...
        open (bool): A boolean indicating whether the comment box is open.
    """
...
def parse_lines(text: str) -> list[Line]:
    ...
    for i, line in enumerate(text.split("\n")):
        lines.append(Line(line, "", i, False))
    return lines
...
def page_commenter(state: State) -> Page:
    ...
    Returns:
        Page: The page displaying the code lines for commenting.
    """
    line_elements = [Header("Code Commenter")]
    for line in state.comments:
        line_elements.append(
            Row(
                PreformattedText(line.original),
                Button(
                    "Comment " + str(line.index),
                    "comment",
                    arguments=Argument("index", line.index),
                ),
                line.comment,
            )
        )
        if line.open:
            line_elements.append(line.comment)

    return Page(state, line_elements)
...
def save_comment(state: State, index: int, comment_box: str) -> Page:
    ...
    state.comments[index].open = False
    return page_commenter(state)
