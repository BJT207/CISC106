...
def make_todo_toggle(completed: bool, index: Argument) -> Button:
    """
    ...
    Args:
        ...
        index (Argument): The index of the to-do item in the list.
    Returns:
        Button: A button that displays a checked or unchecked box based on the completion status.
    """
    if completed:
        return Button("☑️", "toggle_complete", index)
    else:
        return Button("🔲", "toggle_complete", index)
...
def add_todo(state: State) -> Page:
    ...
    return Page(
        ...
        [
            ...
            Row(
                "Difficulty:",
                SelectBox("new_difficulty", DIFFICULTY_RATINGS, "⭐"),
            ),
